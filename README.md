...
Cortex exposes a [Remote Read API] which allows access to the stored series. The `remote-read` subcommand of `cortextool` allows interacting with it's API, to find out which series are stored.
...
This command will run against your Grafana instance and will download its dashboards and then extract the Prometheus metrics used in it's queries. The output is a JSON file.
